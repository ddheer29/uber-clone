      ...
      >
        {onboarding.map((item) => (
          <View key={item.id} className="flex items-center justify-center p-5">
            ...
            <View className="flex flex-row mt-10 justify-center items-center w-full">
              <Text className="text-black text-3xl font-bold mx-10 text-center">{item.title}</Text>
            </View>
            <Text className="text-[#858585] text-lg font-JakartaBold mx-10 mt-3 text-center">{item.description}</Text>
          </View>
        ))}
      </Swiper>
      <CustomButton
